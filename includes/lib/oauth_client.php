 ...
 * oauth_client.php
 *
 * @(#) $Id: oauth_client.php,v 1.187 2023/06/06 20:47:37 mlemos Exp $
 * PHP-OAuth-API (http://www.phpclasses.org/oauth-api)
 *
 */
...
<class>
  ...
  <version>@(#) $Id: oauth_client.php,v 1.187 2023/06/06 20:47:37 mlemos Exp $</version>
  <copyright>Copyright (C) Manuel Lemos 2012 - 2022 and beyond</copyright>
...
{metadocument}
  <variable>
    ...
    <value>0</value>
    <documentation>
      <purpose>Control whether debug messages are displayed as part
        of the current script output.</purpose>
      <usage>Set this variable to <booleanvalue>1</booleanvalue> if
        you need to see the debug messages output.</usage>
    </documentation>
  </variable>
{/metadocument}
*/
...
{metadocument}
  <variable>
    <name>oauth_user_agent</name>
    <type>STRING</type>
    <value></value>
    <documentation>
      <purpose>Define user agente name used to send HTTP requests.</purpose>
      <usage>Change this variable only if it is necessary that the
        OAuth class sends requests to HTTP servers with a different
        identification name.</usage>
    </documentation>
  </variable>
{/metadocument}
*/
  var $oauth_user_agent = 'PHP-OAuth-API (http://www.phpclasses.org/oauth-api $Revision: 1.187 $)';
...
{metadocument}
  <variable>
    ...
    <type>STRING</type>
    <value>oauth_session</value>
    <documentation>
      <purpose>Define the name of the cookie that the class uses to
        store the PHP session name.</purpose>
      <usage>Change this variable only if it is necessary that the
        OAuth class uses a different cookie name to avoid
        collision of names of cookies used for other purposes.</usage>
    </documentation>
  </variable>
{/metadocument}
*/
...
{metadocument}
  <variable>
    <name>session_name_prefix</name>
    <type>STRING</type>
    <value>OAUTH_</value>
    <documentation>
      <purpose>Define the string that the class uses as prefix to
        name PHP session variables.</purpose>
      <usage>Change this variable only if it is necessary that the
        OAuth class uses a different prefix string to avoid
        collision of names of session variables used for other
        purposes.</usage>
    </documentation>
  </variable>
{/metadocument}
...
{metadocument}
  <variable>
    ...
    <value>_</value>
    <documentation>
      <purpose>Define a list of URLs that may need to be changed
        replacing the beggining of the URL to another URL
        beginning.</purpose>
      <usage>Change this variable only if it is necessary that the
        OAuth class replaces the beggining of the URLs of the
        OAuth server or API calls.</usage>
    </documentation>
  </variable>
{/metadocument}
*/
  ...
  Function OutputDebug($message)
  {
    if($this->debug)
    {
      ...
      elseif($this->display_debug)
        echo wp_kses($message,WPJOBPORTAL_ALLOWED_TAGS)."\n";
      else
        error_log($message);
    }
    return(true);
...
<p>It was not possible to authorize the application.<?php
        ...
        {
?>
<br>Authorization error: <?php echo wp_kses(HtmlSpecialChars($this->authorization_error),WPJOBPORTAL_ALLOWED_TAGS);
        }
?></p>
<?php
      }
...
<?php
        ...
        {
?>
<br>Error: <?php echo wp_kses(HtmlSpecialChars($this->access_token_error),WPJOBPORTAL_ALLOWED_TAGS);
        }
?></p>
<?php
      }
...
<?php
        ...
        {
?>
<br>Access token: <?php echo wp_kses(HtmlSpecialChars($this->access_token),WPJOBPORTAL_ALLOWED_TAGS);
          if(IsSet($this->access_token_secret))
          {
?>
<br>Access token secret: <?php echo wp_kses(HtmlSpecialChars($this->access_token_secret),WPJOBPORTAL_ALLOWED_TAGS);
          }
        }
?></p>
<?php
        ...
        {
?>
<p>Access token expiry: <?php echo esc_html($this->access_token_expiry); ?> UTC</p>
<?php
        }
      }
?>
